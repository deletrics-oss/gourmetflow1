...
export default function Balcao() {
  ...
  const addToCart = (item: MenuItem) => {
    ...
    if (existingItem) {
      ...
      setCart([...cart, { ...item, quantity: 1, finalPrice: price }]);
    }
  ...
  const finalizeSale = async () => {
    ...
    try {
      ...
      await supabase.rpc('log_action', {
        ...
        p_details: { total, items: cart.length, customer: customerName }
      });
  ...
  return (
    <div className="min-h-screen bg-background p-4">
      <div className="max-w-7xl mx-auto">
        <h1 className="text-3xl font-bold mb-6 text-foreground">Balcão - Atendimento Rápido</h1>
          ...
          <div className="lg:col-span-2 space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {filteredItems.map(item => (
                <Card key={item.id} className="p-4 hover:shadow-lg transition-shadow cursor-pointer" onClick={() => addToCart(item)}>
